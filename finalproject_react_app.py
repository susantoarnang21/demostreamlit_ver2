...
# 💬 LangGraph ReAct Chatbot (Colab Version)
...
st.set_page_config(page_title="🚗 GAIKINDO Sales Chatbot", page_icon="🤖", layout="wide")
...
# Judul halaman
st.markdown('<div class="title">🚗 GAIKINDO Sales Insight Chatbot</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Chat dengan data wholesales & retail otomotif Indonesia (2023–2025)</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "📁 Unggah file PDF GAIKINDO", type=["pdf"], accept_multiple_files=True
    )
    build_index = st.button("🚀 Bangun Database Informasi")
...
if build_index:
    if not google_api_key:
        st.error("Masukkan API key terlebih dahulu ⚠️")
        st.stop()

    if not uploaded_files:
        st.warning("Unggah minimal satu file PDF 📄")
        st.stop()

    st.info("⏳ Sedang memproses dokumen... Harap tunggu sebentar.")
    all_docs = []
    ...
    st.success("✅ Database PDF GAIKINDO berhasil dibuat!")
...
# --- INPUT CHAT ---
prompt = st.chat_input("💬 Ketik pertanyaan Anda tentang data GAIKINDO...")

if prompt:
    if not st.session_state.chain:
        st.warning("Bangun database terlebih dahulu sebelum chatting 🧠")
        st.stop()
